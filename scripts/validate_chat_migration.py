...
class ChatMigrationValidator:
    ...
    def validate_quantabes_chat(self) -> bool:
        ...
        for user_id in user_ids:
            ...
            if user:
                ...
                logger.info(f"User in chat: {user.first_name} {user.last_name} ({user.user_id})")
            else:
        ...
        if found_john and found_eric:
            logger.info("✅ Quantabes chat validation PASSED - Contains both John Wang and Eric Mueller")
            return True
        else:
            missing = []
            if not found_john:
            ...
            logger.error(f"❌ Quantabes chat validation FAILED - Missing: {', '.join(missing)}")
            return False
    ...
    def validate_migration_completeness(self) -> Dict:
        ...
        try:
            ...
            if not self.source_db_path.exists():
                results["errors"].append(f"Source database not found: {self.source_db_path}")
                results["validation_passed"] = False
                return results
            
            # Check if target database exists
            if not self.target_db_path.exists():
                results["errors"].append(f"Target database not found: {self.target_db_path}")
                results["validation_passed"] = False
                return results
            ...
            results["stats"]["migration_rate"] = round((target_chat_count / source_chat_count) * 100, 2) if source_chat_count > 0 else 0
            ...
            if target_chat_count == 0:
                ...
                results["warnings"].append(f"Not all chats migrated: {target_chat_count}/{source_chat_count}")
            ...
            if chats_without_users > 0:
                results["warnings"].append(f"{chats_without_users} chats have no associated users")
            ...
            if not chats_schema:
                ...
                expected_columns = ['chat_id', 'display_name', 'users']
                actual_columns = [col[1] for col in chats_schema]
                missing_columns = [col for col in expected_columns if col not in actual_columns]
                
                if missing_columns:
                    results["errors"].append(f"Missing columns in chats table: {missing_columns}")
                    results["validation_passed"] = False
                
                results["stats"]["table_columns"] = actual_columns
    ...
    def run_full_validation(self) -> bool:
        ...
        for i, chat in enumerate(samples, 1):
            logger.info(f"Sample {i}: {chat['display_name']} (ID: {chat['chat_id']}) - {len(chat['user_details'])} users")
