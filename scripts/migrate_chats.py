...
class ChatMigrator:
    ...
    def get_chats_from_source(self) -> List[Dict]:
        ...
        try:
            with sqlite3.connect(str(self.source_db_path)) as conn:
                ...
                chats = []
                for row in rows:
                    chat_id, display_name, chat_identifier, service_name, handle_ids_str = row
                    
                    # Parse handle_ids 
                    handle_ids = []
                    if handle_ids_str:
                        handle_ids = [int(h) for h in handle_ids_str.split(',') if h.strip()]
    ...
    def convert_handle_ids_to_user_ids(self, 
                                      chats: List[Dict], 
                                      handle_mapping: Dict[int, str]) -> List[Dict]:
        """
        ...
        for chat in chats:
            ...
            for handle_id in handle_ids:
                if handle_id in handle_mapping:
                    ...
                    logger.warning(f"No user found for handle_id {handle_id} in chat {chat['chat_id']}")
    ...
    def migrate_chats(self) -> bool:
        ...
        if inserted_count == len(target_chats):
            ...
            logger.error(f"Migration incomplete: {inserted_count}/{len(target_chats)} chats inserted")
            return False
    ...
    def get_migration_stats(self) -> Dict:
        ...
        try:
            ...
            target_stats = {
                ...
                "chats_without_users": len([c for c in target_chats if not c['user_ids']])
            }
            ...
            return {
                ...
                "migration_success_rate": round(
                    (target_stats["migrated_chats"] / source_stats["total_chats"]) * 100, 2
                ) if source_stats["total_chats"] > 0 else 0
            }
