...
def validate_implementation():
    ...
    print(f"   Successful extractions: {successful_extractions}/{len(messages)} ({(successful_extractions/len(messages)*100):.1f}%)")
    ...
    decoded_messages = [m for m in messages if m['text_source'] == 'attributed_body_decoded']
    
    for i, message in enumerate(decoded_messages[:5], 1):
        print(f"   {i}. \"{message['extracted_text']}\" (from me: {message['is_from_me']})")
    ...
    success_criteria = [
        ...
        successful_extractions / len(messages) > 0.8  # 80% of sample extracted successfully
    ]
