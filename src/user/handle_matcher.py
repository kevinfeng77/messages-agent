...
class HandleMatcher:
    ...
    def match_handle_to_user(self, handle_id: int, handle_id_value: str) -> Optional[User]:
        """
        ...
        if existing_user:
            logger.debug(f"Found existing user for handle_id {handle_id}: {existing_user}")
        ...
        logger.info(f"No contact match for handle_id {handle_id} ({handle_id_value}), creating new user")
    ...
    def _looks_like_phone(self, value: str) -> bool:
        ...
        return len(digits) >= 10 and (value.startswith("+") or any(c.isdigit() for c in value))
