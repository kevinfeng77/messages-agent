...
class QueryManager:
    ...
    async def search_with_center_node(self, query: str, center_node_uuid: str, limit: int = 10) -> List[Edge]:
        """
        ...
        results = await self.graphiti.search(query, center_node_uuid=center_node_uuid, limit=limit)
    ...
    async def node_search(self, query: str, limit: int = 5) -> List[Node]:
        ...
        search_results = await self.graphiti._search(query=query, config=node_search_config)
        logger.info(f"Node search for '{query}' returned {len(search_results.nodes)} results")
        return search_results.nodes
    
    async def get_auto_center_node_search(self, query: str, limit: int = 10) -> List[Edge]:
        """
        ...
        reranked_results = await self.search_with_center_node(query, center_node_uuid, limit)
        
        logger.info(f"Auto center node search for '{query}' returned {len(reranked_results)} results")
...
async def example_usage():
    ...
    try:
        ...
        center_results = await manager.get_auto_center_node_search("California Attorney General")
