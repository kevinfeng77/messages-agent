...
class MessageMigration:
    ...
    def __init__(self, db_path: Path):
        ...
        self.backup_path = self.db_path.with_suffix(f".backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def analyze_migration_scope(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "remaining_to_migrate": needs_migration - already_migrated if has_extracted_column else needs_migration
            }
    ...
    def migrate_messages(self, batch_size: int = 1000, max_batches: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i in range(0, total_to_migrate, batch_size):
                ...
                logger.info(f"Processing batch {batch_count}, messages {i+1}-{min(i+batch_size, total_to_migrate)}")
                ...
                if batch_count % 10 == 0:
                    ...
                    logger.info(f"Progress: {processed}/{total_to_migrate} ({success_rate:.1f}% success rate)")
            ...
            final_stats = {
                ...
                "success_rate_percent": round((successful_extractions / processed) * 100, 2) if processed > 0 else 0,
...
def migrate_database(db_path: str, create_backup: bool = True, 
                    batch_size: int = 1000, max_batches: Optional[int] = None) -> bool:
    """
    ...
    try:
        ...
        # Perform migration
        results = migration.migrate_messages(batch_size=batch_size, max_batches=max_batches)
