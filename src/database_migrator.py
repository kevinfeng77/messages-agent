...
class DatabaseMigrator:
    ...
    def __init__(self, source_db_path, target_db_path, contacts_db_path=None, addressbook_db_path=None):
        self.source_db_path = source_db_path
        self.target_db_path = target_db_path
        self.contacts_db_path = contacts_db_path or "/Users/{}/Library/Contacts/accounts.accountdb".format(
            os.getenv("USER", "")
        )
        # Default to iCloud AddressBook database
        self.addressbook_db_path = addressbook_db_path or "/Users/{}/Library/Application Support/AddressBook/Sources/0E9330B9-85E5-48C9-ACF9-4EE9217D8F4A/AddressBook-v22.abcddb".format(
            os.getenv("USER", "")
        )
    ...
    def create_target_schema(self):
        ...
        with sqlite3.connect(self.target_db_path) as conn:
            ...
            cursor.execute("CREATE INDEX idx_message_id ON messages_with_contacts(message_id)")
            cursor.execute("CREATE INDEX idx_handle_id ON messages_with_contacts(handle_id)")
            cursor.execute("CREATE INDEX idx_date ON messages_with_contacts(date)")
            cursor.execute("CREATE INDEX idx_phone_email ON messages_with_contacts(phone_email)")
            cursor.execute("CREATE INDEX idx_is_from_me ON messages_with_contacts(is_from_me)")
            cursor.execute("CREATE INDEX idx_contact_name ON messages_with_contacts(contact_full_name)")
    ...
    def migrate_data(self, limit=None):
        ...
        with sqlite3.connect(self.source_db_path) as source_conn:
            with sqlite3.connect(self.target_db_path) as target_conn:
                ...
                logger.info(f"Executing migration query{'with limit ' + str(limit) if limit else ''}...")
    ...
    def _build_contact_lookup(self):
        ...
        addressbook_root = f"/Users/{os.getenv('USER', '')}/Library/Application Support/AddressBook"
        ...
        if os.path.exists(sources_dir):
            for source_dir in os.listdir(sources_dir):
                source_path = os.path.join(sources_dir, source_dir, "AddressBook-v22.abcddb")
        ...
        for db_path in source_databases:
            try:
                with sqlite3.connect(db_path) as conn:
                    ...
                    logger.info(f"Database {os.path.basename(os.path.dirname(db_path))}: {phone_count} phones, {email_count} emails")
    ...
    def get_migration_stats(self):
        ...
        with sqlite3.connect(self.target_db_path) as conn:
            ...
            # Messages by type
            cursor.execute("SELECT is_from_me, COUNT(*) FROM messages_with_contacts GROUP BY is_from_me")
            for row in cursor.fetchall():
                key = 'sent_messages' if row[0] else 'received_messages'
                stats[key] = row[1]
            
            # Unique contacts
            cursor.execute("SELECT COUNT(DISTINCT phone_email) FROM messages_with_contacts WHERE phone_email IS NOT NULL")
            stats['unique_contacts'] = cursor.fetchone()[0]
            
            # Service types
            cursor.execute("SELECT service_type, COUNT(*) FROM messages_with_contacts GROUP BY service_type")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate Messages database to joined format")
    parser.add_argument("--source", default="./data/chat_copy.db", help="Source Messages database")
    parser.add_argument("--target", default="./data/messages_joined.db", help="Target database path")
    ...
    parser.add_argument("--stats-only", action="store_true", help="Show stats only, don't migrate")
