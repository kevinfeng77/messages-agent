...
class MessageDecoder:
    ...
    def decode_attributed_body(self, attributed_body: bytes) -> Optional[str]:
        ...
        try:
            ...
            if text:
                ...
                logger.debug(f"Successfully decoded with NSKeyedUnarchiver: {text[:50]}...")
                return text
            ...
            logger.warning(f"Failed to decode attributedBody of length {len(attributed_body)}")
            return None
    ...
    def _scan_for_text_after_nsstring(self, data: bytes, nsstring_idx: int) -> Optional[str]:
    ...
    def _extract_string_from_archive_data(self, data: bytes) -> Optional[str]:
        ...
        try:
            ...
            while i < len(data) - 4:
                ...
                if i + 4 < len(data):
                    try:
                        length = struct.unpack('>I', data[i:i+4])[0]  # Big-endian 4-byte int
                        if 0 < length < 10000 and i + 4 + length <= len(data):  # Reasonable length
                ...
                if i + 2 < len(data):
                    try:
                        length = struct.unpack('>H', data[i:i+2])[0]  # Big-endian 2-byte int
                        if 0 < length < 1000 and i + 2 + length <= len(data):  # Reasonable length
        ...
        try:
            ...
            if potential_strings:
                ...
                for s in potential_strings:
                    ...
                    if not any(artifact in s.lower() for artifact in [
                        'nsstring', 'nsattributed', 'nsmutable', 'nsarchive',
                        'streamtyped', 'nskeyedarchiver'
                    ]):
...
def extract_message_text(text: Optional[str], attributed_body: Optional[bytes]) -> Optional[str]:
    """
